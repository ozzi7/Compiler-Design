...
public class RegisterCountPrinter {
	...
	public static String toString(Ast ast, String indent) {
		RegisterCountPrinter rcp = new RegisterCountPrinter();
		rcp.dump(ast, indent);
		return rcp.sb.toString();
	}
	
	private StringBuilder sb = new StringBuilder();
	private Visitor vis = new Visitor();
	private RegisterCountUpdater registerCountUpdater = new RegisterCountUpdater();
	...
	protected void dump(Ast ast, String indent) {
		...
		int regCount = registerCountUpdater.getCount(ast);
		if(regCount > 0)
		{
			sb.append(" [" + registerCountUpdater.getCount(ast) + "]\n");
		}
		else
		{
			sb.append("\n");
