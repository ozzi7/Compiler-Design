...
// TODO: (Part a) declare appropriate parser rules   
//       (Part b) declare appropriate rewriting rules
...
classDecl
	:	'class' Identifier ('extends' Identifier)? '{' declSeq '}' 
	;
...
methodDecl
	:	methodHeading methodBody
	;

methodHeading
	:	(type | 'void') Identifier '(' formalParam? ')'
	;
...
formalParam
	...
	;
stmt
	:	assignmentOrMethodCall | ifStmt | whileStmt | returnStmt
	;

assignmentOrMethodCall
	:	identAccess (methodCall | assignment)
	|	writeMethodCall
	;
	
writeMethodCall
	:	( ('write' | 'writef' | 'writeln') methodCallTail) ';'
	;
...
assignment
	:	 '=' (  expr | 'new' qualifiedType | ('read' | 'readf') '(' ')') ';'
	;
	
identAccess
	:	(Identifier methodCallTail? | 'this')  selectorSeq
	;

selectorSeq
	:	(fieldOrMethodSelector | elemSelector)*
	;
...
fieldOrMethodSelector
	:	'.' Identifier methodCallTail?
	;
...
qualifiedType
	:	Identifier '(' ')' | Identifier elemSelector | primitiveType elemSelector
	;
...
noSignFactor
	:	identAccess | integer | FloatNumber | BooleanLiteral | '(' expr ')' | '!' factor 
	|	'(' refType ')' noSignFactor | 'null'
	;
	
refType
	:	Identifier | Identifier '[' ']' | 'int' '[' ']' | 'float' '[' ']' | 'boolean' '[' ']'
	;
